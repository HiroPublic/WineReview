...
| S05 | テースティング入力・レビュー生成準備 | ワイン概要確認、タイプ別スライダー、タグ、自由メモ、初回生成テキストの入力 |
...
flowchart TD
    ...
    S03 --> S04["S04 評価入力"]
    S04 --> S05["S05 テースティング入力・レビュー生成準備"]
...
### 4.5 S05 テースティング入力・レビュー生成準備

#### 目的

Notionから取得したワイン概要を確認しながら、タイプ別スライダー、印象タグ、料理相性タグ、自由メモを入力し、AIへ渡すテースティング情報を構造化する。あわせて、初回生成テキストを調整してレビュー候補生成の準備を行う。
...
タイプごとに最大8個を表示し、選択数は最大3個とする。

- 白ワイン: 柑橘、青リンゴ、洋梨、ミネラル、すっきり、まろやか、上品、爽やか
- 赤ワイン: 赤い果実、黒い果実、スパイス、樽の香り、しっかり、なめらか、力強い、コクがある
- ロゼワイン: いちご、赤い果実、花の香り、すっきり、華やか、軽やか、飲みやすい、上品
- スパークリング: きめ細かい泡、爽快、柑橘、青リンゴ、すっきり、クリーミー、上品、乾杯向き

#### 料理相性タグ

初期状態は折りたたみ表示とする。開いた場合のみ次のタグを表示し、選択数は最大2個とする。
...
スライダーとタグで拾いきれない感想を任意入力する。

プレースホルダー例:

`香りが良く、酸味がきれい。和食にも合わせやすかった。`
...
- ユーザーのテースティング入力
  ...
  - 料理相性タグ
  - 自由メモ
- 定型テキスト1を初期値としてユーザーが編集した初回生成テキスト全文
...
Xcodeのローカル/限定的な開発用署名で実機へインストールする場合、provisioning profileが短期間で失効することがある。個人利用MVPでは、Mac上の補助スクリプトで署名期限を監視し、期限切れ前に再ビルド・再インストールする運用を許容する。

- 実機にインストール済みの`WineReview.app`に埋め込まれた`embedded.mobileprovision`の`ExpirationDate`を監視する。
- 期限までの残り日数が2日以下になった場合、Mac側で警告を出し、自動的に再ビルドと実機への再インストールを試行する。
- 再ビルド前に、現在の埋め込みprofileと同じUUIDのローカルprofileがあれば退避し、新しい署名生成を促す。
- 再インストールには、対象iPhoneがMacから`devicectl`で`available (paired)`として見える状態が必要。
- Macが常時稼働していない前提で、LaunchAgentはログイン時と1時間ごとに期限チェックを行う。
- ビルドと再インストールが完了しても期限が延びなかった場合は成功扱いにせず、同じ期限に対して12時間は再試行しない。
- 期限まで十分な日数が残っている場合は、チェックのみで終了し、アプリ本体やNotionデータには影響を与えない。
- 自動更新に失敗した場合はMac側の通知とログで確認できるようにする。
...
- 実機開発運用では、署名期限チェックがログイン時と1時間ごとに実行され、残り2日以下で再ビルド・再インストールが試行される。
