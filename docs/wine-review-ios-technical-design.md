...
| 実機運用 | `xcodebuild`、`xcrun devicectl`、`launchd` | 開発署名profileの期限監視、再ビルド、実機再インストール |
...
Mac上の`refresh_ios_profile.py`で、開発用provisioning profileの期限監視と更新フローを確認する。

1. `python3 scripts/refresh_ios_profile.py --threshold-days 2 --device-name "Your iPhone Name" --dry-run`を実行し、`ExpirationDate`と残り日数が表示されることを確認する。
2. `--force --dry-run`を実行し、再ビルドが必要な分岐に入るが、実際のビルドとインストールを行わないことを確認する。
3. 対象iPhoneが`xcrun devicectl list devices`で`available (paired)`として見えることを確認する。
4. `python3 scripts/install_profile_refresh_agent.py --device-name "Your iPhone Name" --dry-run`でLaunchAgent plistの内容を確認する。
5. profile期限が延びない状況を再現した場合、`Wine Review profile not extended`通知が出て、失効が近い場合でも失効前に再試行できる短い待機時間が設定されることを確認する。
6. LaunchAgent登録後、`launchctl print gui/<uid>/<label>`で`run interval = 3600 seconds`と`runatload`が有効であることを確認する。

## 15. 実機署名更新設計

### 15.1 目的

Xcodeのローカル/限定的な開発用署名で実機へインストールした場合、provisioning profileが短期間で失効し、iPhone上でアプリが利用できない旨のメッセージが表示されることがある。これを避けるため、Mac側で期限を監視し、期限切れ前に自動で再署名ビルドと再インストールを行う。

### 15.2 スクリプト

`scripts/refresh_ios_profile.py`が次の処理を担当する。

- `DerivedData/DeviceRun/Build/Products/Debug-iphoneos/WineReview.app/embedded.mobileprovision`から`ExpirationDate`を読み取る。
- 残り日数が`--threshold-days`以下、または`--force`指定時に更新処理へ進む。
- `--device-id`が指定されていなければ、`--device-name`に一致する`available (paired)`な実機を`xcrun devicectl list devices`から探す。
- まず既存profileを残したまま`xcodebuild -allowProvisioningUpdates`で`generic/platform=iOS`向けDebugビルドを行う。
- `xcodebuild`が`No Accounts`で失敗した場合は、Xcodeを起動してから1回だけ再試行する。
- ビルド後も古いprofile期限のままなら、その時点でだけローカルのcached profileを退避して再ビルドする。失敗時は退避したprofileを元に戻す。
- `xcrun devicectl device install app`で対象iPhoneへ再インストールする。
- 更新開始、成功、失敗をmacOS通知とログへ出す。通知を抑止する場合は`--no-notify`を使う。
- ビルドとインストール後にprofile期限が十分延びたことを確認できた場合だけ成功通知を出す。
- 期限が延びなかった場合は状態ファイルを記録し、同じ期限に対しても失効前に再試行できるよう待機時間を短く調整する。
...
端末名や端末IDは、`--device-name` / `--device-id`の代わりに`WINE_REVIEW_DEVICE_NAME` / `WINE_REVIEW_DEVICE_ID`でも指定できる。

### 15.3 LaunchAgent

`scripts/install_profile_refresh_agent.py`で、実行環境に合わせたLaunchAgent plistを`~/Library/LaunchAgents/`へ生成する。公開リポジトリには端末名やローカル絶対パスを含むplistを置かない。

- `RunAtLoad`: ログインまたはLaunchAgent読み込み時に1回チェックする。
- `StartInterval = 3600`: Macが起きている間、おおむね1時間ごとにチェックする。
- `StartCalendarInterval = 08:30`: 8:30にMacが起きていれば追加でチェックする。
- `EnvironmentVariables`: `HOME` / `USER` / `LOGNAME`を明示し、Xcodeや通知がユーザー環境を参照しやすくする。
- 標準出力と標準エラーは`~/Library/Logs/`配下へ出力する。
...
- Macがスリープ中の実行は保証されない。`RunAtLoad`と1時間ごとの`StartInterval`で、起動中の実行機会を増やす。
- iPhoneがMacからワイヤレスまたはUSBで`available (paired)`として見えていない場合、再インストールは失敗する。
- 期限まで十分な日数が残っている場合、スクリプトは期限確認のみで終了する。
- Xcodeが同じlocal provisioning profileを再利用すると、再ビルドと再インストールが成功しても期限が延びない場合がある。このときは古いprofileを退避し、一定時間の再試行抑制を入れる。
- 起動ページでは、実機アプリに埋め込まれた`embedded.mobileprovision`の期限を読み取り、24時間未満なら赤字で警告する。
- profile更新後、iPhone側で開発元の信頼が必要になる場合がある。
...
- 実機署名更新スクリプトはMac側の開発運用補助であり、アプリバンドルには含めない。
