...
5. profile期限が延びない状況を再現した場合、`Wine Review profile not extended`通知が出て、同じ期限に対して12時間は再試行しないことを確認する。
...
- 再ビルド前に、現在埋め込まれているprofileと同じUUIDのローカルprovisioning profileが見つかった場合は退避して、新しいprofile生成を促す。
- `xcodebuild -allowProvisioningUpdates`で`generic/platform=iOS`向けDebugビルドを行い、新しい署名を生成する。
...
- 期限が延びなかった場合は状態ファイルを記録し、同じ期限に対して12時間は再試行しない。
