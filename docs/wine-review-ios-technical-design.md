...
5. LaunchAgent登録後、`launchctl print gui/<uid>/<label>`で`run interval = 3600 seconds`と`runatload`が有効であることを確認する。
...
- `xcodebuild -allowProvisioningUpdates`で実機向けDebugビルドを行う。
